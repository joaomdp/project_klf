  ...
  return (
    <div className={`fixed inset-0 z-[2000] flex items-center justify-center p-4 md:p-8 transition-all duration-300 ${isClosing ? 'bg-black/0' : 'bg-black/95 backdrop-blur-xl animate-in fade-in'}`}>
      ...
      <div className={`relative w-full max-w-xl bg-[#0B0411] border border-white/10 shadow-[0_0_100px_rgba(0,0,0,1)] flex flex-col max-h-[90vh] transition-all duration-500 overflow-hidden ${isClosing ? 'opacity-0 scale-95 translate-y-12' : 'opacity-100 scale-100 translate-y-0 animate-in zoom-in-95'}`}>
        ...
        {/* Cabeçalho */}
        <div className="p-6 border-b border-white/5 bg-gradient-to-r from-[#6366F1]/10 to-transparent shrink-0 flex items-center justify-between">
          <div>
            <h2 className="font-orbitron font-black text-xl text-white uppercase tracking-tight mb-1">NOVA LIGA</h2>
            <span className="text-[9px] font-black text-[#6366F1] uppercase tracking-widest">SISTEMA DE FUNDAÇÃO</span>
          </div>
          {!isSubmitting && (
            <button onClick={triggerClose} className="w-9 h-9 flex items-center justify-center bg-white/5 border border-white/10 text-gray-500 hover:text-[#6366F1] hover:border-[#6366F1]/40 transition-all">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-3">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 py-3 px-5 text-[13px] text-white font-black tracking-tight focus:outline-none focus:border-[#6366F1]/60 transition-all placeholder:text-gray-800" 
                required 
            ...
            <div className="space-y-3">
              ...
              <div 
                ...
                className={`w-full bg-white/5 border-2 border-dashed p-6 flex flex-col items-center justify-center gap-4 cursor-pointer transition-all group ${leagueImage ? 'border-[#6366F1]/50' : 'border-white/5 hover:border-[#6366F1]/40'}`}
              >
                ...
                {leagueImage ? (
                  ...
                  <div className="text-center space-y-3">
                    <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mx-auto group-hover:scale-110 group-hover:bg-[#6366F1]/10 transition-all">
                      <i className="fa-solid fa-upload text-lg text-gray-700 group-hover:text-[#6366F1]"></i>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div onClick={() => !isSubmitting && setNewLeagueFormat('continuo')} className={`relative p-4 border-2 transition-all cursor-pointer ${newLeagueFormat === 'continuo' ? 'bg-[#6366F1]/[0.06] border-[#6366F1]/60 shadow-[0_0_20px_rgba(94,108,255,0.05)]' : 'bg-white/5 border-transparent hover:border-white/10'}`}>
                  <div className="flex items-start gap-3">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 ${newLeagueFormat === 'continuo' ? 'border-[#6366F1]' : 'border-gray-800'}`}>
                      {newLeagueFormat === 'continuo' && <div className="w-2 h-2 bg-[#6366F1] rounded-full animate-pulse shadow-[0_0_8px_#6366F1]"></div>}
                    </div>
                    <div>
                ...
                <div onClick={() => !isSubmitting && setNewLeagueFormat('limitado')} className={`relative p-4 border-2 transition-all cursor-pointer ${newLeagueFormat === 'limitado' ? 'bg-[#6366F1]/[0.06] border-[#6366F1]/60 shadow-[0_0_20px_rgba(94,108,255,0.05)]' : 'bg-white/5 border-transparent hover:border-white/10'}`}>
                  <div className="flex items-start gap-3">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 ${newLeagueFormat === 'limitado' ? 'border-[#6366F1]' : 'border-gray-800'}`}>
                      {newLeagueFormat === 'limitado' && <div className="w-2 h-2 bg-[#6366F1] rounded-full animate-pulse shadow-[0_0_8px_#6366F1]"></div>}
                    </div>
                    <div>
            ...
            <div className="pt-3 flex flex-col items-center gap-4 pb-4">
              ...
              <button 
                ...
                className={`group relative w-full py-4 font-orbitron font-black text-sm uppercase tracking-[0.3em] transition-all duration-500 ${
                  submitStatus === 'success' ? 'bg-green-600 text-white' : submitStatus === 'error' ? 'bg-red-600 text-white' : isSubmitting ? 'bg-gray-800 text-gray-600' : 'bg-[#6366F1] text-black shadow-[0_20px_50px_rgba(94,108,255,0.4)] hover:scale-[1.01] active:scale-95'
                }`}
              >
              ...
              <button type="button" onClick={triggerClose} className="text-[9px] font-black text-gray-700 uppercase tracking-[0.3em] hover:text-white transition-all underline underline-offset-4 decoration-[#6366F1]/20">ABORTAR OPERAÇÃO</button>
            </div>
          </form>
        </div>
      </div>
